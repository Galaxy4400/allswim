...
class Currency {
  ...
  async init() {
    this.localCurrency = this.getLocalCurrency();
  ...
  async getCurrentRate(from = 'USD') {
    try {
      ...
      const response = await fetch(`https://api.frankfurter.dev/v1/latest?from=${from}&to=${this.localCurrency}`);
  ...
  convertPriceInHTML() {
    ...
    for (const block of currencyBlocks) {
      ...
      block.innerText = value ? this.toLocalFormat(converted) : this.getLocalCurrency();
    }
  }
}
