...
export async function imagesResponsive() {
  ...
  const sizes = [320, 640, 1280, 1920, 2560, 3840, 5120, 7680];
  ...
  if (app.imgNames) {
    files = files.filter((file) => {
      ...
      return app.imgNames.some((pattern) => minimatch(fileName, pattern));
    });
  }
  ...
  for (const file of files) {
    ...
    const imgSizes = [...sizes.filter((size) => size < metadata.width), metadata.width];

    await fs.mkdir(outDir, { recursive: true });

    for (const width of imgSizes) {
